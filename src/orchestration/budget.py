...
class BudgetedAgentAdapter(IAgentAdapter):
    ...
    This is deliberately a preflight boundary. Provider billing remains external;
    Kalyx treats each model/tool call as a governed resource and never allows an
    LLM loop to run indefinitely.
    """

    def __init__(
        self,
        delegate: IAgentAdapter,
        *,
        max_calls: int = 8,
    ...
    def generate_structured(
        ...
        if self.calls_used >= self.max_calls:
            raise AgentBudgetExceeded(
                f"Orchestrator compute budget exhausted: {self.calls_used}/{self.max_calls} calls"
            )
