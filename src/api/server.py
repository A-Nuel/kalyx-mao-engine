...
class IdentityAuthorizationMiddleware(BaseHTTPMiddleware):
    """Authenticate API resource scope before handlers can read organisation data."""
    async def dispatch(self, request, call_next):
        ...
        if path.startswith("/api/organisations"):
            ...
            try:
                ...
                if len(parts) == 2 and parts[1] == "organisations":
                    rows = db.conn.execute("SELECT DISTINCT tenant_id FROM organisations").fetchall()
                    # A tenant selector is required when identity enforcement is enabled;
                    # membership is checked against the persisted principal, not the header.
                    if os.getenv("KALYX_IDENTITY_AUTH", "false").strip().lower() in {"1", "true", "yes", "on"}:
                        if not tenant:
...
def create_and_run_mission(request: MissionRequest, x_api_key: str | None = Header(default=None, alias="X-API-Key"),
                          x_principal_id: str | None = Header(default=None, alias="X-Principal-ID"),
                          x_tenant_id: str | None = Header(default=None, alias="X-Tenant-ID")) -> Dict[str, Any]:
...
def organisations() -> list[Dict[str, Any]]:
    db = _db()
    try:
        tenant = os.getenv("KALYX_IDENTITY_AUTH", "false").strip().lower() in {"1", "true", "yes", "on"}
        if tenant:
            raise HTTPException(status_code=400, detail="Tenant scope must be supplied via X-Tenant-ID")
...
def ledger(org_id: str, limit: int = Query(default=200, ge=1, le=1000)) -> Dict[str, Any]:
    ...
    try:
        _org_id_or_404(db, org_id)
        l = SqliteLedger(db, initial_treasury=0); entries = l.get_entries()[-limit:]
...
def events(org_id: str, limit: int = Query(default=200, ge=1, le=1000)) -> list[Dict[str, Any]]:
    db = _db()
    try:
        _org_id_or_404(db, org_id); store = SqliteEventStore(db, verify_on_startup=True); all_events = store.get_events()
        related_ids = {org_id}; task_rows = db.conn.execute("SELECT id FROM tasks WHERE org_id = ?", (org_id,)).fetchall(); task_ids = {row["id"] for row in task_rows}; related_ids.update(task_ids)
        if task_ids:
            placeholders = ",".join("?" for _ in task_ids); proposal_rows = db.conn.execute(f"SELECT id FROM proposals WHERE task_id IN ({placeholders})", tuple(task_ids)).fetchall(); proposal_ids = {row["id"] for row in proposal_rows}; related_ids.update(proposal_ids)
            if proposal_ids:
                placeholders = ",".join("?" for _ in proposal_ids); receipt_rows = db.conn.execute(f"SELECT id FROM execution_receipts WHERE proposal_id IN ({placeholders})", tuple(proposal_ids)).fetchall(); related_ids.update(row["id"] for row in receipt_rows)
...
def pause(org_id: str, x_api_key: str | None = Header(default=None, alias="X-API-Key")) -> Dict[str, Any]:
    ...
    try:
        ...
        _operator_event(db, org_id, "ORG_PAUSED", org["state"], "PAUSED"); db.conn.execute("UPDATE organisations SET state = ? WHERE id = ?", ("PAUSED", org_id)); db.conn.commit()
...
def resume(org_id: str, x_api_key: str | None = Header(default=None, alias="X-API-Key")) -> Dict[str, Any]:
    ...
    try:
        ...
        _operator_event(db, org_id, "ORG_RESUMED", "PAUSED", "EXECUTING"); db.conn.execute("UPDATE organisations SET state = ? WHERE id = ?", ("EXECUTING", org_id)); db.conn.commit()
