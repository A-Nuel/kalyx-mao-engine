...
class SqliteLedger:
    ...
    def verify_conservation(self) -> bool:
        ...
        cursor.execute(
            """
            SELECT 
                COALESCE(SUM(CASE WHEN from_account != ? THEN amount ELSE 0 END), 0) as total_debited,
                COALESCE(SUM(amount), 0) as total_credited
            FROM ledger_entries
            """,
            (SYSTEM_MINT,)
        )
        # All non-mint accounts balance sum
        cursor.execute(
            """
            WITH accounts AS (
        ...
        net_balance = cursor.fetchone()["net_balance"]
        return net_balance == total_minted


class SqliteEventStore:
    """
    Append-only cryptographic event store persisted in SQLite.
    Survives process restart and verifies integrity on startup.
    """
...
class SqliteRepository:
    ...
    def save_organisation(self, org: Organisation) -> None:
        with self.db.conn:
            self.db.conn.execute(
                """
                INSERT INTO organisations (id, mission, treasury_balance, state, created_at)
                VALUES (?, ?, ?, ?, ?)
    ...
    def load_organisation(self, org_id: str, ledger: Optional[Any] = None) -> Optional[Organisation]:
        ...
        for ar in cursor.fetchall():
            ...
            org.agents[ar["id"]] = AgentRecord(
                ...
                credit_balance=ar["credit_balance"],
        ...
        with self.db.conn:
            self.db.conn.execute(
                ...
                (
                    ...
                    agent.authority_ceiling,
                    json.dumps([at.value for at in agent.allowed_action_types]),
                    agent.status.value,
    ...
    def save_task(self, task: Task, org_id: str) -> None:
        with self.db.conn:
            self.db.conn.execute(
                ...
                ON CONFLICT(id) DO UPDATE SET
                    status = excluded.status,
                    output_evidence = excluded.output_evidence
                """,
                (
                    task.id,
                    org_id,
                    ...
                    task.objective,
                    task.allocated_credits,
                    task.status.value,
                    json.dumps(task.output_evidence) if task.output_evidence else None,
        ...
        with self.db.conn:
            self.db.conn.execute(
                ...
                (
                    ...
                    proposal.expected_value_score,
                    proposal.risk_assessment,
                    proposal.rationale,
                    proposal.get_content_hash(),
        ...
        with self.db.conn:
            self.db.conn.execute(
                ...
                (
                    ...
                    decision.violated_rule_id,
                    decision.violated_rule_description,
                    json.dumps(decision.evaluated_rules),
        ...
        with self.db.conn:
            self.db.conn.execute(
                ...
                (
                    receipt.id,
                    receipt.proposal_id,
                    receipt.authorization_token,
                    receipt.action_type.value,
                    receipt.target,
                    receipt.http_status,
                    receipt.raw_response_hash,
                    json.dumps(receipt.raw_output),
