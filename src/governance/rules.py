...
class PolicyRule(ABC):
    ...
    @abstractmethod
    def evaluate(self, proposal: ActionProposal, agent: AgentRecord, org: Organisation) -> Optional[str]:
...
class TreasuryBalanceRule(PolicyRule):
    ...
    def evaluate(self, proposal: ActionProposal, agent: AgentRecord, org: Organisation) -> Optional[str]:
        if proposal.requested_credits > org.treasury_balance:
            return f"Requested {proposal.requested_credits} credits exceeds available treasury ({org.treasury_balance})"
        return None
