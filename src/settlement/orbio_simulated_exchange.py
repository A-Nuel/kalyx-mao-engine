...
def compute_operation_fingerprint(operation: ConsequentialOperation) -> str:
    """Compute canonical cryptographic fingerprint of operation parameters."""
    ...
    fp_data = {
        ...
        "network": (params.get("network") or "").lower(),
        "exchange_contract": (params.get("exchange_contract") or params.get("recipient") or "").lower(),
        "payment_token": (params.get("payment_token") or params.get("token_contract") or "").lower(),
        "credit_token": (params.get("credit_token") or "").lower(),
        "beneficiary": (params.get("beneficiary_bytes32") or params.get("beneficiary") or "").lower(),
        "usdg_in": int(params.get("usdg_in", operation.amount or 0)),
        "min_credit_out": int(params.get("min_credit_out", 0)),
        "purchase_intent_hash": params.get("purchase_intent_hash"),
    }
    return hashlib.sha256(canonical_json(fp_data).encode("utf-8")).hexdigest()
...
class SimulatedOrbioExchangeProvider(ConsequentialProviderAdapter):
    ...
    def __init__(
        self,
        *,
        name: str = "orbio-exchange-simulated",
    ...
    def compute_operation_fingerprint(self, operation: ConsequentialOperation) -> str:
        """Compute canonical cryptographic fingerprint of operation parameters."""
        return compute_operation_fingerprint(operation)
    ...
    def execute(self, operation: ConsequentialOperation) -> ProviderExecutionResult:
        ...
        params = operation.parameters if isinstance(operation.parameters, dict) else {}
        current_fingerprint = self.compute_operation_fingerprint(operation)

        # Idempotent replay with canonical fingerprint verification
        if key in self._records:
            ...
            if rec.fingerprint and rec.fingerprint != current_fingerprint:
                return ProviderExecutionResult(
                    ...
                    raw_response={
                        "error": "idempotency_key_collision_fingerprint_mismatch",
                        "simulated": True,
                        "existing_fingerprint": rec.fingerprint,
                        "presented_fingerprint": current_fingerprint,
                    },
                    error_message=f"Idempotency key '{key}' collision: operation fingerprint does not match original record",
                )
        ...
        if key in self._timeout_keys or op_id in self._timeout_keys:
            if key in self._timeout_background_success or op_id in self._timeout_background_success:
                # Preflight economic checks before settling background success
                bg_usdg = int(params.get("usdg_in", 0))
                bg_min_credit = int(params.get("min_credit_out", 0))
                bg_org = operation.organisation_id
                bg_avail = self.get_usdg_balance(bg_org)
                if bg_usdg <= 0:
                    self._settle_failure(operation, params, "usdg_in must be positive", fingerprint=current_fingerprint)
                elif bg_avail < bg_usdg:
                    self._settle_failure(
                        operation,
                        params,
                        f"Insufficient USDG: available={bg_avail}, required={bg_usdg}",
                        fingerprint=current_fingerprint,
                    )
                else:
                    bg_credit = self._compute_credit_out(bg_usdg)
                    if bg_credit < bg_min_credit:
                        self._settle_failure(
                            operation,
                            params,
                            f"Slippage: credit_out={bg_credit} < min_credit_out={bg_min_credit}",
                            fingerprint=current_fingerprint,
                        )
                    else:
                        self._settle_success(
                            operation,
                            params,
                            credit_out=bg_credit,
                            usdg_spent=bg_usdg,
                            fingerprint=current_fingerprint,
                        )
            elif key in self._timeout_background_failure or op_id in self._timeout_background_failure:
                self._settle_failure(operation, params, "Simulated background failure", fingerprint=current_fingerprint)
            return ProviderExecutionResult(
                provider_name=self.name,
                operation_id=op_id,
        ...
        if failure_reason:
            rec = self._settle_failure(operation, params, failure_reason, fingerprint=current_fingerprint)
        ...
        if usdg_in <= 0:
            rec = self._settle_failure(operation, params, "usdg_in must be positive", fingerprint=current_fingerprint)
            return self._result_from_record(rec, op_id)
        ...
        rec = self._settle_success(operation, params, credit_out=credit_out, usdg_spent=usdg_in, fingerprint=current_fingerprint)
        return self._result_from_record(rec, op_id)
    ...
    def status(
        ...
        **kwargs: Any,
        ...
        raw_status = dict(record.raw) if record.raw else {}
        raw_status.update({
            ...
            "simulated": True,
            "chain_id": record.chain_id or (record.raw or {}).get("chain_id"),
            "network": record.network or (record.raw or {}).get("network"),
            "exchange_contract": record.exchange_contract,
            "beneficiary": record.beneficiary,
            "purchase_intent_hash": (record.raw or {}).get("purchase_intent_hash"),
        ...
        return ProviderStatusResult(
            ...
            error_message=record.raw.get("error") if record.outcome != ProviderOutcome.SUCCESS else None,
